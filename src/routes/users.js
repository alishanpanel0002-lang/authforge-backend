...
router.post('/:app_id/create', async (req, res) => {
  ...
  const insert = { app_id: req.params.app_id, username, password_hash, email: email || null,
  ...
  if (license_id) {
    ...
    insert.license_id = license_id;
    const { data, error } = await supabase.from('app_users').insert([insert]).select('id, username, email, created_at').single();
    if (error) return res.status(400).json({ error: error.message });
